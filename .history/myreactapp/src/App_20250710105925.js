  ...
  return (
    <div className="App">
      ...
      <ul className="countries-list">
        {data.map((country) => (
          <li key={country.id}>
            {country.code} - {country.name} - {country.capital} -{country.continent}
            {country.population ? ` - Population: ${country.population}` : ''}
            {country.area ? ` - Area: ${country.area} km²` : ''}
            {country.region ? ` - Region: ${country.region}` : ''}
            {country.subregion ? ` - Subregion: ${country.subregion}` : ''}
            {country.languages ? ` - Languages: ${country.languages.join(', ')}` : ''}
            {country.currencies ? ` - Currencies: ${country.currencies.join(', ')}` : ''}
            {country.flag ? <img src={country.flag} alt={`${country.name} flag`} style={{ width: '50px', height: '30px' }} /> : ''}
          </li>
        ))}
      </ul>
